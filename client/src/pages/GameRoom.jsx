...
import PieceSetupBoard from "../components/PieceSetupBoard";
...
export default function GameRoom() {
  ...
  // Setup phase state (freestyle piece placement)
  const [setupProgress, setSetupProgress] = useState({ white: false, black: false });
  const [mySetupSubmitted, setMySetupSubmitted] = useState(false);
  ...
  useEffect(() => {
    ...
    socket.on(
      "game-state",
      ({ fen: serverFen, pgn: serverPgn, players, status: gameStatus, setupProgress: progress }) => {
        gameRef.current = new Chess(serverFen);
        setFen(serverFen);
        setPgn(serverPgn);
        setPlayerCount(players || 0);
        setStatus(gameStatus || "waiting");
        if (progress) setSetupProgress(progress);
        if (gameStatus !== "setup") setMySetupSubmitted(false);
    ...
    socket.on("setup-start", () => {
      setStatus("setup");
      setSetupProgress({ white: false, black: false });
      setMySetupSubmitted(false);
      showToast("Both players joined. Arrange your pieces!", "info");
    });

    socket.on("setup-progress", (progress) => {
  ...
  // Send chosen back-rank arrangement to the server
  const handleSubmitSetup = (arrangement) => {
    const socket = socketRef.current;
    if (!socket) return;
    socket.emit("submit-setup", { roomId, arrangement });
    setMySetupSubmitted(true);
  };
  ...
  const getStatusText = () => {
    ...
    if (status === "setup") {
      if (role === "spectator") {
        return "Players are arranging their pieces...";
      }
      return mySetupSubmitted ? "Waiting for opponent's setup..." : "Arrange your pieces";
    }
    if (status === "started") {
      if (role === "spectator") {
        return "Spectating";
        ...
        <div style={styles.boardSection}>
          <div style={styles.statusArea}>
            ...
            <div style={styles.statusDetail}>
              ...
              {status === "setup" ? (
                <span>
                  White: {setupProgress.white ? "✓ ready" : "arranging..."} · Black:{" "}
                  {setupProgress.black ? "✓ ready" : "arranging..."}
                </span>
              ) : null}
            </div>
          </div>
          ...
          {/* Board / Setup */}
          {status === "setup" && role !== "spectator" && !mySetupSubmitted ? (
            <PieceSetupBoard color={role} onSubmit={handleSubmitSetup} />
          ) : status === "setup" ? (
            <div style={{ ...styles.boardContainer, ...styles.setupWaiting }}>
              <p>
                {role === "spectator"
                  ? "Waiting for both players to finish arranging their pieces..."
                  : "Your formation is locked in. Waiting for your opponent..."}
              </p>
            </div>
          ) : (
            <>
